...
# Variable global para el número máximo de hilos del CPU
    ...
    with zipfile.ZipFile(zip_file_name, 'w') as zip_file:
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            for i, audio_file_path in enumerate(audio_files):
                executor.submit(process_audio, i, audio_file_path, zip_file, segment_duration)

    messagebox.showinfo("Info", f"Audio segments have been saved to {zip_file_name}.")

def process_audio(i, audio_file_path, zip_file, segment_duration):
